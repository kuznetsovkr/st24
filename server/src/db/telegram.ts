...
export const upsertTelegramB2BSubscriber = async (
  input: TelegramSubscriberInput
): Promise<TelegramSubscriberRow> => {
  const result = await query(
    `
      INSERT INTO telegram_b2b_subscribers (chat_id, username, first_name, last_name, language_code, chat_type, is_active)
      VALUES ($1, $2, $3, $4, $5, $6, TRUE)
      ON CONFLICT (chat_id) DO UPDATE
        SET username = EXCLUDED.username,
            first_name = EXCLUDED.first_name,
...
export const listTelegramB2BSubscribers = async (): Promise<TelegramSubscriberRow[]> => {
  const result = await query(
    `
      SELECT chat_id, username, first_name, last_name, language_code, chat_type, is_active, created_at, updated_at
      FROM telegram_b2b_subscribers
      WHERE is_active = TRUE;
    `
  );
...
export const deactivateTelegramB2BSubscriber = async (chatId: string) => {
  await query(
    `
      UPDATE telegram_b2b_subscribers
      SET is_active = FALSE,
          updated_at = NOW()
      WHERE chat_id = $1;
    `,
